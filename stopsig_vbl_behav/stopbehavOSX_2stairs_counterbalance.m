...
%%% Adapted for Speech & Brain lab use by Saloni Krishnan 05/2017, added
%%% sound

clear all;
% output version
script_name='Stopbehav_verbal: optimized SSD tracker';
script_version='4';
revision_date='9-5-17';
...
try  % goes with catch at end of script
    ...
    for n=1:numDevices,
        ...
        elseif findstr(devices(n).transport,'ADB') & findstr(devices(n).usageName,'Keyboard'),
            inputDevice=n;
        elseif findstr(devices(n).transport,'SPI') & findstr(devices(n).usageName,'Keyboard'), %inserted for MacBook Air
            inputDevice=n;
        end;
    end;
    ...
    freq1=22254;
    nrchannels1 = size(wave,1);    
    % Default to auto-selected default output device:
    deviceid = -1;
    % Requestlatency mode 2, which used to be the best one in our measurement:
    reqlatencyclass = 2; % class 2 empirically the best, 3 & 4 == 2
    % Initialize driver, request low-latency preinit:
    InitializePsychSound(1);
    % Open audio device for low-latency output:
    pahandle1 = PsychPortAudio('Open', deviceid, [], reqlatencyclass, freq1, nrchannels1);
    %Play the sound
    ...
    Screen('DrawText',w,'After you see see a +',100,175);
    Screen('DrawText',w,'You will see words like HAXP',100,225);
    Screen('DrawText',w,'Say them as FAST as you can',100,300);
    Screen('DrawText',w,'when you see the word.',100,350);
    Screen('DrawText',w,'But if you hear a beep, try very hard',100,425);
    Screen('DrawText',w,'to STOP yourself from saying the word.',100,475);
...
%                     Screen('DrawText',w,'+', CrossPosX, CrossPosY);
%                     Screen('TextSize',w,WordSize);
%                     Screen('TextFont',w,'Arial');
                ...
                if Seeker(Pos,3)~=2, %% ie this is not a NULL event		
                    ...
                    Screen('TextFont',w,'Arial');
                    if (Seeker(Pos,4)==0),
                        DrawFormattedText(w,'HAXP', 'center', 'center');
                    else
                        DrawFormattedText(w,'FINT', 'center', 'center');
                    end;
                    ...
                    %set up listening
                    freq2 = 44100;
                    pahandle2 = PsychPortAudio('Open', [], 2, 2, freq2, 2, [], 0.02);
                    ...
                    while level < triggerlevel && noresp && (GetSecs-arrow_start_time < arrow_duration)
                        ...
                        if Seeker(Pos,3)==1 && GetSecs - arrow_start_time >=Seeker(Pos,6)/1000 && notone,
                            ...
                            PsychPortAudio('GetAudioData', pahandle2, 10);
                            PsychPortAudio('Start', pahandle2, 0, 0, 1);
                            while GetSecs<Seeker(Pos,14)+1 && level < triggerlevel && noresp,
                                %set up listening
                                [audiodata offset overflow tCaptureStart]= PsychPortAudio('GetAudioData', pahandle2);
                                % Compute maximum signal amplitude in this chunk of data:
                                if ~isempty(audiodata)
